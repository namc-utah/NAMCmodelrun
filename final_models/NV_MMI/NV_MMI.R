NV_MMI_model<-function(test_bugs,test_preds,rf_model){
  ####adjust metrics for natural variability and rescale
  INSET.raw=test_bugs$INSET
  ...
  PER_EPHEA.raw=test_bugs$PER_EPHEA
  ...
  PER_PLECA.raw=test_bugs$PER_PLECA
  ...
  SHDIVER.raw=test_bugs$SHDIVER #SHDIVER not adjusted by natural gradients
  ...
  test_bugs.rs=data.frame(INSET.rs, PER_EPHEA.rs, SHDIVER.rs, PER_CFA.rs, PER_PLECA.rs, NONSET.rs, CLINGER.rs, row.names=row.names(test_bugs))
  MMI=rowSums(test_bugs.rs)/7
  return(MMI)
}
