...
# Make sure randomForest library is running:
library(randomForest);
library(vegan)
# Establish working directory (wd): 
###wd = "C:/Users/Christian/999_trash/20170713_westwideOEmodel_runNewDataThrough/"

setwd("\\\\share1.bluezone.usu.edu\\miller\\\\buglab\\OE_Modeling\\NAMC_Supported_OEmodels\\West_Wide_Model\\NewWestWide_NONMidgeModel\\Model Files\\")

# Load the westwide non-midge 2017 model files:

load(paste0("My.RF.Model.Version1.Rdata"))
# Source the predict script:
source(paste0("model.predict.RanFor.4.2.r"));

# Read in your predictor data and BMI data:
### Note: you'll need to adjust the naming of two predictor variables: PctSalLake and Precip8110.  For the model to work, you'll need to change the name of each so that they do NOT have WS at the end of the name.

Boxdata <- ("\\\\share1.bluezone.usu.edu\\miller\\\\buglab\\OE_Modeling\\NAMC_Supported_OEmodels\\West_Wide_Model\\NewWestWide_NONMidgeModel\\InputsAndResults\\CurrentRun")
setwd(Boxdata) 
getwd()
test.preds <- read.csv("WW18_Preds.csv",row.names="SampleID",header=T)
test.bugs <- read.csv("WW18_Bugs.csv",row.names="SampleID",header=T)


# Check that there is no missing data in your predictor set (summary should show that all are TRUE):
summary(complete.cases(test.preds))

# Check row alignment of bug and predictor data (summary should show that all are TRUE):
summary(row.names(test.preds)==row.names(test.bugs))

# makes predictions for test data;
OE.assess.test<-model.predict.RanFor.4.2(bugcal.pa[,c(2:222)],grps.final,preds.final,ranfor.mod=rf.mod.best.from.rFVR, prednew=test.preds, bugnew=test.bugs, Pc=0.5, Cal.OOB=FALSE);

# look at O/E scores, for all samples:
head(OE.assess.test$OE.scores)

# Create a histogram of O/E scores:
windows()
hist(OE.assess.test$OE.scores$OoverE,col="blue", main = "OoverE")

# Export csv file with OE scores:
write.csv(OE.assess.test$OE.scores,paste0("WW18_OEscores.csv"))
...
CalPredsModelApplicability=read.csv('Z:\\buglab\\OE_Modeling\\NAMC_Supported_OEmodels\\West_Wide_Model\\NewWestWIde_NONMidgeModel\\Model Files\\CalPredsModelApplicability.csv')
...
TestPredsModelApplicability=read.csv('Z:\\buglab\\OE_Modeling\\NAMC_Supported_OEmodels\\West_Wide_Model\\NewWestWIde_NONMidgeModel\\InputsAndResults\\CurrentRun\\TestPredsModelApplicability.csv')
...
final=cbind(OE.assess.test$OE.scores,out.flag90,TestPredsModelApplicability,outlier.preds.test.std)

write.csv(final, file="WW18_OEscores.csv")
