...
library(vegan)
#looking at trait contribution
#the top 5 traits are correlated, but they can still show a life history
#signal: poor-swimming clingers are more prevalent at reference sites.
...
top5
traits_only3<-as.data.frame(lapply(traits_only2[,1:ncol(traits_only2)],as.numeric))
traits_scaled <- scale(na.omit(traits_only3[,names(traits_only3) %in% top5]))
pca <- prcomp(traits_scaled, scale = TRUE)
biplot(pca)
...
O_long=plyr::join(O_long,traits_only[,names(traits_only) %in% c(top5,'taxon','Survive_desiccation_yes')])
ProbOs_long=plyr::join(ProbOs_long,traits_only[,names(traits_only) %in% c(top5,'taxon','Survive_desiccation_yes')])
site_cols=colnames(O_long)[1:(ncol(O_long)-8)]
traitcols=colnames(O_long)[(ncol(O_long)-5):ncol(O_long)]
Psite_cols=colnames(ProbOs_long)[1:(ncol(ProbOs_long)-8)]
Ptraitcols=colnames(ProbOs_long)[(ncol(ProbOs_long)-5):ncol(ProbOs_long)]
