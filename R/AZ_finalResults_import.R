...
# ---------------------------------------------------------------
modelResults=read.csv("AIM2021resultsMod.csv")
modelResults = modelResults[,c("ActivityID","TotalIndividualsRaw","IBI", "InvertReg")] # what about TotalInd_ why is this over 300 in some cases... should this be the fixedcount we use instead of down below?
modelResults$modelId=ifelse(modelResults$InvertReg=="cold",169,
                            ifelse(modelResults$InvertReg=="warm",236,NA
                                   ))
modelResults=setNames(modelResults,c("sampleId","fixedCount","MMI","InvertReg","modelId"))
...
finalResults=dplyr::left_join(finalResults,invasives,by="sampleId")
finalResults$InvasiveInvertSpecies[is.na(finalResults$InvasiveInvertSpecies)]<-"Absent"
