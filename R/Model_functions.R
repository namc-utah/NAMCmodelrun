...
AREMP_MMI_model<-function(bugnew,prednew,CLING_rich.rf,DIPT_rich.rf,LLT_rich.rf,NON_INSECT_rich.rf,PER_EPT.rf,PER_INTOL.rf,rf_models,mdeg_metrics_adj_cal,ref_metrics_adj){
  ...
  for(n in 1:length(rf_models)){
    ...
    if(model$rsq[1500]>=0.1){
      metric_prd=predict(model,newdata=prednew[,preds.final])}
    if(model$rsq[1500]<0.1){
...
NV_MMI_model<-function(bugnew,prednew,CLINGER.rf,INSET.rf,NONSET.rf,PER_CFA.rf,PER_EPHEA.rf,PER_PLECA.rf){
  ...
  INSET.pred=predict(INSET.rf, newdata=prednew[,preds.final], type="response")
  ...
  PER_CFA.pred=predict(PER_CFA.rf, newdata=prednew[,preds.final], type="response")
  ...
  PER_EPHEA.pred=predict(PER_EPHEA.rf, newdata=prednew[,preds.final], type="response")
  ...
  NONSET.pred=predict(NONSET.rf, newdata=prednew[,preds.final], type="response")
  ...
  CLINGER.pred=predict(CLINGER.rf, newdata=prednew[,preds.final], type="response")
  ...
  PER_PLECA.pred=predict(PER_PLECA.rf, newdata=prednew[,preds.final], type="response")
