...
#traits and %I and %D
#Fisher Exact test
#t test for continuous trait states
#this came from the FoFe_script... file

rbind(O_long,ProbOs_long)
O_for_traits=O_long
O_long2=O_long[,names(O_long)[659:(ncol(O_long)-1)] %in% names(trait_table)]
O_long2$status='Ref'
P_for_traits=ProbOs_long
P_long2=ProbOs_long[,names(ProbOs_long)[352:(ncol(ProbOs_long)-1)] %in% names(trait_table)]
P_long2$status='Prob'
trait_table=read.csv('C://Users//andrew.caudillo.BUGLAB-I9//Box//NAMC//Research Projects//AIM//IncreaserDecreaser_OE//Updated_w_modelObj//New_for_SFS//Maximized_OTUmatrix_trimmed.csv')
...
categorical_cols <- names(P_everything)[352:389]
...
final_traits=names(P_everything)[352:389]
names(P_everything)
#-----------------------------------
...
clipr::write_clip(as.data.frame(fisher_results))
...
Prob_site_cols=names(P_everything)[1:349]
Prob_Trait_cols=names(P_everything)[352:389]
...
P_for_RF=P_everything[,names(P_everything) %in% c(names(P_everything)[1:350], categorical_cols,'OTU','Regional_Response')]
P_for_RF=P_for_RF[,names(P_for_RF) %in% c(names(P_everything)[1:350],good_traits,'OTU','Regional_Response')]
...
O_for_RF=O_w_Traits[,names(O_w_Traits) %in% c(names(O_w_Traits)[1:657], categorical_cols,'OTU','Regional_Response')]
O_for_RF=O_for_RF[,names(O_for_RF) %in% c(names(O_for_RF)[1:657], good_traits,'OTU','Regional_Response')]

maxxed_OTUs2=maxxed_OTUs
...
#I = 1, D=0
P_for_RF$Regional_Response=as.factor(ifelse(P_for_RF$Regional_Response=='Increaser',1,0))
swapP_for_RF$Regional_Response=as.factor(ifelse(swapP_for_RF$Regional_Response=='Increaser',0,1))
library(randomForest)
set.seed(42)

ID_trait_mod=randomForest(Regional_Response~.,data=P_for_RF,ntree=500)
mod_imp=as.data.frame(importance(ID_trait_mod))
mod_imp$trait=row.names(mod_imp)
mod_imp=mod_imp[order(mod_imp$MeanDecreaseGini, decreasing = T),]
mod_imp[1:5,]$trait
ID_trait_mod

varImpPlot(ID_trait_mod,main='')
savp(10,8,'C://Users//andrew.caudillo.BUGLAB-I9//Box//NAMC//Research Projects//AIM//IncreaserDecreaser_OE//Updated_w_modelObj//New_for_SFS//I_D_traitmod1.png')
windows(10,8)
I_prob=ID_trait_mod$votes[,"1"]
eps <- 1e-15
I_prob <- pmax(pmin(I_prob, 1 - eps), eps)

# model log-likelihood
ll_model <- sum(
  I_prob * log(I_prob) +
    (1 - I_prob) * log(1 - I_prob)
)

# null model probability
I_prob_null <- mean(I_prob)

# null log-likelihood
ll_null <- sum(
  I_prob * log(p_null) +
    (1 - I_prob) * log(1 - p_null)
)
...
pseudo_r2
oobs=ID_trait_mod$votes[,"1"]
pROC::roc(P_for_RF$Regional_Response,oobs)

#removing top 5 predictors
top5_ID_trait_mod_preds=c('Develop_slow_season',
'Shape_streamline',
'Thermal_pref_Cool_warm_eurythermal_5_30_C',
'Feed_prim_abbrev_CG',
'Female_disp_abbrev_High')
set.seed(42)
ID_trait_mod2=randomForest(Regional_Response~Shape_streamline+Feed_prim_abbrev_CG+
                             Max_body_size_abbrev_Large+Develop_slow_season+Occurance_drift_rare,data=P_for_RF,ntree=500)
...
oobs=ID_trait_mod2$votes[,"1"]
pROC::roc(P_for_RF$Regional_Response,oobs)
#better plotting
imp_df <- as.data.frame(importance(ID_trait_mod2))
imp_df$Predictor <- rownames(imp_df)

# Create custom labels
custom_labels <- c("Shape_streamline" = "Streamlined Body",
                   "Develop_slow_season" = "Slow Development",
                   "Feed_prim_abbrev_CG" = "Collector-Gatherer",
                   "Max_body_size_abbrev_Large" = "Large Body Size",
                   'Ocurrance_drift_rare' = 'Rarely Drifts')

# Plot with ggplot2
ggplot(imp_df, aes(x = reorder(Predictor, MeanDecreaseGini), y = MeanDecreaseGini)) +
  geom_point(size=5) +
  #geom_hline(xintercept = imp_df$MeanDecreaseGini, color = "lightgray", linetype = "dashed") +
  coord_flip() +
  scale_x_discrete(labels = custom_labels) +
  labs(x = "Predictors", y = "Mean Decrease Gini") +
  theme_classic()+
...
savp(10,8,'C://Users//andrew.caudillo.BUGLAB-I9//Box//NAMC//Research Projects//AIM//IncreaserDecreaser_OE//Updated_w_modelObj//New_for_SFS//I_D_traitmod_trimmed.png')
...
topvarsPA=c("Precip8110",        "MSST_mean08091314", "WsAreaSqKm",        "LONG",
            "MWST_mean08091314")
topvarsCount=c("PctSalLake",        "MSST_mean08091314", "Precip8110",        "WsAreaSqKm",
             "SWs")
vars=topvarsPA
...
ggplot(vimp_summary, aes(x = reorder(Predictor, Mean), y = Mean)) +
  ...
  theme_classic()+
   scale_x_discrete(labels=c('% Sulfur in Basin',
                             'Basin Area (Sq. km)',
                            '30-year Mean Annual Precip',
                             '% Saline Sediment in Basin',
                             'Mean Summer Stream Temp'))+
  theme(axis.text = element_text(size=14),
        axis.title = element_text(size=15))


savp(10,8,'C://Users//andrew.caudillo.BUGLAB-I9//Box//NAMC//Research Projects//AIM//IncreaserDecreaser_OE//Updated_w_modelObj//New_for_SFS//I_D_PctMod_vimp_trimmed.png')
#PA
windows(10,8)
ggplot(vimp_summary, aes(x = reorder(Predictor, Mean), y = Mean)) +
  geom_col(fill = "dodgerblue4", alpha = 0.7) +
...
set.seed(42)
#setting the index for where predictors start
pred_index<-names(Osite_trait_pct)[c(41:54,57,61)]

#force the predictors to be numbers, not strings, as SQL shows
train <- Osite_trait_pct
trait_names=names(train)[1:39]
#only for pct MRF
train=train[complete.cases(train[,trait_names]),]
...
#           "RunoffWs",          "SWs",               "PctSalLake",               "MAST_mean08091314",
#            "MSST_mean08091314", "MWST_mean08091314",'DOY')
topvarsPA_REF=c("MSST_mean08091314", "Tmean8110Ws",       "LAT",               "WsAreaSqKm",
            "MAST_mean08091314")
topvarsCount_REF=c("LAT",        "MSST_mean08091314", "Tmean8110Ws",        "MAST_mean08091314",
             "WsAreaSqKm")
vars=topvarsCount_REF
formula_string <- paste0(
  "cbind(", paste(trait_names, collapse = ", "), ") ~ ",
  #paste(pred_names,collapse='+'))
  #paste(names(train[,(ncol(train)-13):ncol(train)]),collapse = '+'))
...
savp(10,8,'C://Users//andrew.caudillo.BUGLAB-I9//Box//NAMC//Research Projects//AIM//IncreaserDecreaser_OE//Updated_w_modelObj//New_for_SFS//Reference_I_D_PA_Mod_vimp_trimmed.png')
#PA
windows(10,8)
ggplot(vimp_summary, aes(x = reorder(Predictor, Mean), y = Mean)) +
  geom_col(fill = "dodgerblue4", alpha = 0.7) +
...
PA_traits_ForNMDS_all=PA_traits_ForNMDS_all[,c(1:39,62)]
...
NMDS_PA_sor=vegan::vegdist(PA_traits_ForNMDS_all[,1:39],method='bray',binary=T)
...
savp(10,8,'C://Users//andrew.caudillo.BUGLAB-I9//Box//NAMC//Research Projects//AIM//IncreaserDecreaser_OE//Updated_w_modelObj//New_for_SFS//PA_trait_NMDS.png')
...
PCT_traits_ForNMDS_all=PCT_traits_ForNMDS_all[,c(1:39,62)]
...
NMDS_PCT_sor=vegan::vegdist(PCT_traits_ForNMDS_all[,1:39],method='bray',binary=F)
...
ggplot(NMDS_PCTscores,aes(x=NMDS1,y=NMDS2,fill=status))+geom_point(pch=21,size=3)+
  ...
  theme(
    ...
    axis.ticks=element_blank(),
    legend.position = 'inside',
    legend.position.inside = c(.15,0.25)
  )#+stat_ellipse(level=0.8,type='t')
savp(10,8,'C://Users//andrew.caudillo.BUGLAB-I9//Box//NAMC//Research Projects//AIM//IncreaserDecreaser_OE//Updated_w_modelObj//New_for_SFS//PCT_trait_NMDS.png')
...
NMDS_Count_sor=vegan::vegdist(Count_traits_ForNMDS_all[,1:39],method='bray',binary=F)
...
ggplot(NMDS_Countscores,aes(x=NMDS1,y=NMDS2,fill=status))+geom_point(pch=21,size=3)+
  ...
  theme(
    ...
    axis.ticks=element_blank(),
    legend.position = 'inside',
    legend.position.inside = c(max(NMDS_Countscores$NMDS1)+0.09,0.2)
  )#+stat_ellipse(level=0.8,type='t')
savp(10,8,'C://Users//andrew.caudillo.BUGLAB-I9//Box//NAMC//Research Projects//AIM//IncreaserDecreaser_OE//Updated_w_modelObj//New_for_SFS//Count_trait_NMDS.png')


summary(Psite_trait_pct[,1:39])

summ_mat=apply(Psite_trait_pct[,1:39], 2, function(x) {
